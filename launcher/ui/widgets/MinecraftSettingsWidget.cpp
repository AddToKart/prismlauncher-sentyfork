    ...
    : QWidget(parent), m_instance(std::move(instance)), m_ui(new Ui::MinecraftSettingsWidget)
{
    ...
    connect(m_ui->openGpuSettingsButton, &QPushButton::clicked, this, [] {
        QProcess::startDetached("ms-settings:display-advancedgraphics");
    });
...
void MinecraftSettingsWidget::loadSettings()
{
    ...
    {
        // Populate combo with detected GPUs
        QStringList gpus = HardwareInfo::enumerateAllGpus();
        for (const auto& gpu : gpus) {
            m_ui->gpuPreferenceCombo->addItem(gpu);
        }

        QString gpuPref = settings->get("GpuPreference").toString();
        int idx = 0;
        if (gpuPref == "integrated")
            idx = 1;
        else if (gpuPref == "high_performance")
            idx = 2;
        else if (!gpuPref.isEmpty() && gpuPref != "default") {
            // Try to find the GPU name in the combo
            int found = m_ui->gpuPreferenceCombo->findText(gpuPref);
            if (found >= 0)
                idx = found;
        }
        m_ui->gpuPreferenceCombo->setCurrentIndex(idx);
    }
...
void MinecraftSettingsWidget::saveSettings()
{
...

    {
        ...
        if (performance) {
            ...
            QString gpuPref = "default";
            int idx = m_ui->gpuPreferenceCombo->currentIndex();
            if (idx == 1)
                gpuPref = "integrated";
            else if (idx == 2)
                gpuPref = "high_performance";
            else if (idx >= 3)
                gpuPref = m_ui->gpuPreferenceCombo->currentText();
            settings->set("GpuPreference", gpuPref);
        } else {
            settings->reset("EnableFeralGamemode");
            settings->reset("EnableMangoHud");
            settings->reset("UseDiscreteGpu");
            settings->reset("UseZink");
            settings->reset("GpuPreference");
        }
