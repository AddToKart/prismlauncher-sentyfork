...
struct MojangError {
    static MojangError fromJSON(QByteArray data)
    {
        ...
        if (!out.parseError.error) {
            ...
            out.fullyParsed &= getString(object.value("path"), out.path);
            out.fullyParsed &= getString(object.value("error"), out.error);
            out.fullyParsed &= getString(object.value("errorMessage"), out.errorMessage);
        }
