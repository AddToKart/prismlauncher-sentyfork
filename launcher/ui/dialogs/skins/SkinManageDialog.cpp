...
void SkinManageDialog::on_userBtn_clicked()
{
    ...
    connect(getProfile.get(), &Task::succeeded, this, [profileLoop, profileOut, job, getProfile, &mcProfile, downloadSkin, &failReason] {
        auto doc = QJsonDocument::fromJson(*profileOut);
        if (!doc.isObject()) {
        ...
        auto skinObj = root.value("properties").toArray().first().toObject().value("value").toString();
        auto skinDoc = QJsonDocument::fromJson(skinObj.toUtf8());
        if (!skinDoc.isObject()) {
            failReason = tr("failed to parse get user profile response");
            job->abort();
            profileLoop->quit();
        ...
        auto textures = skinDoc.object().value("textures").toObject();
        auto skin = textures.value("SKIN").toObject();
        mcProfile.skin.url = skin.value("url").toString();
        mcProfile.skin.variant = skin.value("metadata").toObject().value("model").toString("classic");
        if (!mcProfile.skin.url.isEmpty()) {
